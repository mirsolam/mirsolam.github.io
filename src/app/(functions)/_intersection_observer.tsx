...
import { SECONDARY_BACKGROUND, PRIMARY_BACKGROUND, getNavTexts, getSections } from "./_global_functions";
...
export default function initializeObserver(observerRef: RefObject<{main: IntersectionObserver|null, prev: { id: string, theme: string }}>, setActive: Dispatch<SetStateAction<string>>) {
    const options = {
        root: null,
        rootMargin: "0px",
    ...
    const mobileMenuButtonSecondary = document.getElementById("mobile-menu-button-secondary")
    const mobileMenuButtonPrimary = document.getElementById("mobile-menu-button-primary")
    ...
    function intersectionHandler(entries: Array<IntersectionObserverEntry>) {
        entries.forEach((entry) => {
            ...
            const primaryColor = theme === "primary-theme" ? PRIMARY_BACKGROUND.rgb : SECONDARY_BACKGROUND.rgb
            const secondaryColor = theme === "primary-theme" ? SECONDARY_BACKGROUND.rgb : PRIMARY_BACKGROUND.rgb

            navTexts.map(el => {
                const item = el as HTMLElement
                if (entry.boundingClientRect.top <= item.getBoundingClientRect().bottom)
                    if (item.style.color !== secondaryColor && !isNavHidden)
            ...
            if (entry.boundingClientRect.top <= nav!.getBoundingClientRect().bottom) {
                if (theme === observerRef.current.prev.theme && nav!.style.borderImage.includes(`${secondaryColor} 100%`)) {
                    if (entry.intersectionRatio > 0.94) {
                        setActive(id.split("_")[1])
                    }
                    return
                }
                nav!.style.borderImage = `linear-gradient(0deg, ${secondaryColor} ${entry.intersectionRatio * 100}%, ${primaryColor} 0)`
    ...
    observerRef.current.main = new IntersectionObserver(intersectionHandler, options);
